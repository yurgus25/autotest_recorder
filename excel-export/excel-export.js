...
class ExcelExporter {
   ...
   */
  async init() {
    // Дефолтные настройки excelExport на случай, если в storage пусто
    ...
    try {
      ...
      if (result.pluginSettings) {
        ...
        if (oldResult.settings) {
          ...
          // Настройки ещё не инициализированы — используем дефолты
          this.settings = defaultExcelSettings;
          console.log('ℹ️ [ExcelExport] Настройки не найдены в storage, используются значения по умолчанию');
        }
      }
    } catch (error) {
      console.warn('⚠️ [ExcelExport] Не удалось загрузить настройки, используем дефолты:', error);
      this.settings = defaultExcelSettings;
    }
  }
  ...
  prepareTestSteps(actions, runHistory) {
    ...
    actions.forEach((action, index) => {
      ...
      const step = {
        ...
        consoleErrors: this.formatConsoleErrors(stepHistory?.consoleErrors || [])
      };
  ...
  /**
   * Форматирует ошибки консоли для отображения в Excel
   */
  formatConsoleErrors(consoleErrors) {
    if (!consoleErrors || !Array.isArray(consoleErrors) || consoleErrors.length === 0) {
      return '';
    }

    return consoleErrors.map((err, index) => {
      const prefix = err.type === 'error' ? '❌' : err.type === 'warn' ? '⚠️' : '🔴';
      const message = err.message || 'Неизвестная ошибка';
      const timestamp = err.timestamp ? new Date(err.timestamp).toLocaleTimeString('ru-RU') : '';
...
// Для использования в content scripts
