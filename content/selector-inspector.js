// Selector Inspector - shows selectors on mouse hover
// Защита от повторной инъекции
(function() {
  if (window.__autotestSelectorInspector || window.selectorInspector) return;
  window.__autotestSelectorInspector = true;
...
class SelectorInspector {
  constructor() {
    ...
    this.fixedElement = null; // Fixed element
    this.fixedHighlight = null; // Highlight of fixed element
    this.holdTimer = null; // Hold timer
    this.holdStartTime = null; // Hold start time
    this.fixTime = null; // Element fix time (to prevent immediate unfreeze)
    this.holdStartX = null; // X coordinate of hold start
    this.holdStartY = null; // Y coordinate of hold start
    this.zIndexObserver = null; // Z-index change observer
    this.zIndexCheckInterval = null; // Interval for z-index check
    this.activeTimeouts = []; // Array for all setTimeout IDs
    this.analyzeDebounceTimer = null; // Timer for debounce analyzeElement
    this.isActivating = false; // Flag for race conditions protection
    this.currentLang = 'en'; // Current language
    this.init();
  }
  ...
  t(key) {
    const translations = {
      en: {
        title: '🔍 Selector Inspector',
        backToPopup: 'Back to popup',
        hoverElement: 'Hover over an element to view selectors',
        hoverElementShort: 'Hover over an element',
        noText: '(no text)',
        noSelectors: 'No selectors found',
        best: '✓ Best',
        unique: '✓ Unique',
        copy: '📋 Copy',
        copied: '✓ Copied',
        ...
        notFound: '✕ Not found',
        action: 'Action',
        elementFixed: '✓ Element fixed. Click outside to unfreeze.',
        elementUnfixed: 'Hover over an element to view selectors'
      },
      ru: {
        title: '🔍 Инспектор селекторов',
        backToPopup: 'Вернуться к popup',
        hoverElement: 'Наведите курсор на элемент для просмотра селекторов',
        hoverElementShort: 'Наведите курсор на элемент',
        noText: '(нет текста)',
        noSelectors: 'Селекторы не найдены',
        best: '✓ Лучший',
        unique: '✓ Уникальный',
        copy: '📋 Копировать',
        copied: '✓ Скопировано',
        ...
        notFound: '✕ Не найден',
        action: 'Действие',
        elementFixed: '✓ Элемент зафиксирован. Кликните вне окна для снятия фиксации.',
        elementUnfixed: 'Наведите курсор на элемент для просмотра селекторов'
      }
    };
  ...
  async loadLanguage() {
    try {
      ...
      if (result.i18nLang && ['en', 'ru'].includes(result.i18nLang)) {
        this.currentLang = result.i18nLang;
      }
    } catch (e) {
      console.warn('[SelectorInspector] Could not load language:', e);
    }
  }
  ...
  init() {
    ...
    chrome.storage.onChanged.addListener((changes, namespace) => {
      if (namespace === 'local' && changes.i18nLang) {
        this.currentLang = changes.i18nLang.newValue || 'en';
        // Update UI if inspector is active
        if (this.isActive && this.inspectorWindow) {
          this.updateInspectorLanguage();
        }
      }
    });
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      try {
        if (message.type === 'TOGGLE_SELECTOR_INSPECTOR') {
          console.log('📨 [SelectorInspector] Received TOGGLE_SELECTOR_INSPECTOR message');
          this.toggle();
  ...
  activate() {
    ...
    console.log('✅ [SelectorInspector] Inspector activated');
    } finally {
  ...
  deactivate() {
    ...
    console.log('❌ Selector Inspector deactivated');
  }
  ...
  createInspectorWindow() {
    ...
    // Создаем окно инспектора (переменная inspectorEl — не shadowing глобального window)
    const inspectorEl = document.createElement('div');
    inspectorEl.id = 'selector-inspector-window';
    inspectorEl.innerHTML = `
      <div class="inspector-header" id="inspectorHeader">
        <span class="inspector-title">${this.t('title')}</span>
        <div class="inspector-header-actions">
          <button class="inspector-back" id="inspectorBack" title="${this.t('backToPopup')}">←</button>
          <button class="inspector-close" id="inspectorClose">✕</button>
        </div>
      </div>
      <div class="inspector-content">
        <div class="inspector-info">
          <div class="inspector-hint">${this.t('hoverElement')}</div>
        ...
        <div class="inspector-selectors" id="selectorsList">
          <div class="empty-state">${this.t('hoverElementShort')}</div>
        </div>
      </div>
    `;
    ...
    // Сохраняем ссылку на DOM‑элемент окна инспектора
    this.inspectorWindow = inspectorEl;
    ...
    if (header) {
      this.makeDraggable(this.inspectorWindow, header);
    }
    ...
    this.zIndexObserver = new MutationObserver((mutations) => {
      if (!this.inspectorWindow) return;
      const el = this.inspectorWindow;
      const computedStyle = getComputedStyle(el);
      const currentZIndex = el.style.zIndex || computedStyle.zIndex;
      const maxZIndex = 2147483647;
      
      // Если z-index изменился или стал меньше максимального, восстанавливаем
      const currentZIndexNum = parseInt(currentZIndex) || 0;
      if (currentZIndexNum < maxZIndex && el) {
        el.style.setProperty('z-index', maxZIndex.toString(), 'important');
        el.style.setProperty('position', 'fixed', 'important');
    ...
    this.zIndexObserver.observe(this.inspectorWindow, {
      attributes: true,
    ...
    this.zIndexCheckInterval = setInterval(() => {
      const el = this.inspectorWindow;
      if (!this.isActive || !el || !el.parentNode || !document.documentElement) {
        return;
      }
      
      try {
        const computedStyle = getComputedStyle(el);
        const currentZIndex = parseInt(el.style.zIndex || computedStyle.zIndex || '0');
  ...
  startHoldTimer(element, x, y) {
    ...
    console.log('⏱️ Hold timer started, waiting 2 seconds...');
    
    this.holdTimer = setTimeout(() => {
      // 2 seconds passed - fix element
      console.log('✅ 2 seconds passed, fixing element');
      this.fixElement(element);
    }, 2000);
  }

  // Update inspector language when language changes
  updateInspectorLanguage() {
    if (!this.inspectorWindow) return;
    ...
    const backBtn = this.inspectorWindow.querySelector('.inspector-back');
    if (backBtn) backBtn.title = this.t('backToPopup');
    
    // Update hint
    const hint = this.inspectorWindow.querySelector('.inspector-hint');
    if (hint) {
      hint.textContent = this.fixedElement ? this.t('elementFixed') : this.t('hoverElement');
    }
    
    // Update empty state
    const emptyState = this.inspectorWindow.querySelector('.empty-state');
    if (emptyState) {
      emptyState.textContent = this.t('hoverElementShort');
    }
    
    // Update button labels
    this.inspectorWindow.querySelectorAll('.btn-copy').forEach(btn => {
      if (!btn.dataset.copied) btn.textContent = this.t('copy');
    });
    
    this.inspectorWindow.querySelectorAll('.btn-test').forEach(btn => {
      btn.textContent = this.t('test');
    });
    
    // Update badges
    this.inspectorWindow.querySelectorAll('.badge-best').forEach(badge => {
      badge.textContent = this.t('best');
    });
    
    this.inspectorWindow.querySelectorAll('.badge-unique').forEach(badge => {
      badge.textContent = this.t('unique');
    });
    
    // Update action labels
    this.inspectorWindow.querySelectorAll('.selector-action-type').forEach(label => {
      const actionType = label.textContent.replace('Действие: ', '').replace('Action: ', '');
      label.textContent = this.t('action') + ': ' + actionType;
    });
  }
  ...
  fixElement(element) {
    ...
    if (hint) {
      hint.textContent = this.t('elementFixed');
      hint.style.background = '#e8f5e9';
      hint.style.color = '#2e7d32';
    }
    
    console.log('✅ Element fixed:', element);
  }
  ...
  unfixElement() {
    ...
    const hint = document.querySelector('.inspector-hint');
    if (hint) {
      hint.textContent = this.t('hoverElement');
      hint.style.background = '#f5f5f5';
      hint.style.color = '#666';
    }
    
    console.log('❌ Element unfixed');
  }
  ...
  async analyzeElement(element) {
    ...
    elementText.textContent = text ? `"${text.substring(0, 50)}${text.length > 50 ? '...' : ''}"` : this.t('noText');
    ...
    if (selectors.length === 0) {
      selectorsList.innerHTML = `<div class="empty-state">${this.t('noSelectors')}</div>`;
      return;
    }
    ...
    selectorsList.innerHTML = selectors.map(sel => `
      <div class="selector-item ${sel.isUnique ? 'unique' : ''} ${sel.isBest ? 'best' : ''}">
        <div class="selector-header">
          ...
          ${sel.isBest ? `<span class="badge-best">${this.t('best')}</span>` : ''}
          ${sel.isUnique ? `<span class="badge-unique">${this.t('unique')}</span>` : ''}
        </div>
        <div class="selector-value" title="${this.escapeHtml(sel.selector)}">${this.escapeHtml(sel.selector)}</div>
        <div class="selector-actions">
          <button class="btn-copy" data-selector="${this.escapeHtml(sel.selector)}">${this.t('copy')}</button>
          <button class="btn-test" data-selector="${this.escapeHtml(sel.selector)}">${this.t('test')}</button>
        </div>
        ${sel.actionType ? `<div class="selector-action-type">${this.t('action')}: ${sel.actionType}</div>` : ''}
      </div>
    `).join('');
    ...
    selectorsList.querySelectorAll('.btn-test').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        if (found) {
          btn.textContent = this.t('found');
          btn.style.background = '#4CAF50';
        } else {
          btn.textContent = this.t('notFound');
