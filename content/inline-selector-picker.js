...
(function() {
  if (window.__autotestInlineSelectorPicker || window.InlineSelectorPicker) return;
  window.__autotestInlineSelectorPicker = true;
...
class InlineSelectorPicker {
  constructor(element, selectors, options = {}) {
    ...
    // Лучший селектор по мнению встроенного инспектора (SelectorEngine) — используется при «Использовать лучший» и таймауте
    this.engineBestSelector = this.options.engineBestSelector || null;
  ...
  calculateScore(selector) {
    ...
    // tag-text с повышенным приоритетом (текст ссылки при GWT-id)
    if (type === 'tag-text' && typeof selector.priority === 'number' && selector.priority < 5) score += 38;
    ...
    if (type === 'id' && /^#id[a-f0-9]{1,14}$/i.test(sel)) score -= 45;
    if (type === 'id' && /^#rc_select_\d+$/i.test(sel)) score -= 45;
    if (type === 'id' && /^#gwt-uid-\d+$/i.test(sel)) score -= 45;
    if (type === 'id' && /^#gwt-/i.test(sel)) score -= 40;
    ...
    // Простой ID селектор (не GWT id3c)
    if (/^#[\w-]+$/.test(sel) && selector.isUnique && !/^#id[a-f0-9]{1,14}$/i.test(sel) &&
        !/^#rc_select_\d+$/i.test(sel) && !/^#gwt-uid-\d+$/i.test(sel)) score += 15;
    ...
    return `
      ...
      <div class="picker-content">
        ...
        <div class="picker-selectors">
          ${visible.map((sel, idx) => this.buildSelectorOption(sel, idx, this.engineBestSelector ? (sel.selector === this.engineBestSelector.selector) : (idx === 0))).join('')}
        </div>
  ...
  attachHandlers() {
    ...
    // Использовать лучший (по мнению инспектора селекторов, иначе первый в списке пикера)
    const useBestBtn = this.panel.querySelector('#picker-use-best');
    if (useBestBtn) {
      useBestBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        const toUse = this.engineBestSelector || this.selectors[0];
        this.emit('select', toUse);
        this.close();
      });
    }
