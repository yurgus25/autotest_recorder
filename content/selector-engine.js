...
// Защита от повторной инъекции (injectContentScriptsIfNeeded, injectSelectorInspectorScripts)
(function() {
  if (window.__autotestSelectorEngine || window.selectorEngine) return;
  window.__autotestSelectorEngine = true;
...
class SelectorEngine {
   ...
   */
  /**
   * Эвристика «сгенерированный» id (GWT, ExtJS и т.п.): #id3c, короткий hex, gwt-uid-*.
   * Такие id нестабильны между сессиями — селектор по ним хуже, чем по тексту/placeholder/name.
   */
  isLikelyUnstableId(id) {
    if (!id || typeof id !== 'string') return false;
    const s = id.trim();
    if (s.length === 0) return false;
    const lower = s.toLowerCase();
    const stableWords = new Set([
      'main', 'nav', 'menu', 'form', 'home', 'list', 'grid', 'row', 'col', 'app', 'box',
      'btn', 'tab', 'map', 'root', 'header', 'footer', 'content', 'sidebar', 'search', 'filter'
    ]);
    if (stableWords.has(lower)) return false;
    // GWT: id + hex (id3c, id12ab)
    if (/^id[a-f0-9]{1,12}$/i.test(s)) return true;
    // GWT / SmartGWT: rc_select_0, gwt-uid-12, gwt-debug-id-… (номер часто меняется после перезагрузки)
    if (/^rc_select_\d+$/i.test(s)) return true;
    if (/^gwt-uid-\d+$/i.test(s)) return true;
    if (/^gwt-debug-id-\d+$/i.test(s)) return true;
    // gwt-*, ext-gen-*, ext-comp-*
    if (/^(gwt-|ext-gen-|ext-comp-|ember\d)/i.test(s)) return true;
    // длинная hex-строка без разделителей (часто автоген)
    if (/^[a-f0-9]{10,40}$/i.test(s)) return true;
    // короткий «случайный» фрагмент без подчёркиваний/дефисов (осторожно)
    if (s.length >= 3 && s.length <= 8 && /^[a-z][a-z0-9]*$/i.test(s) && !/[._-]/.test(s)) {
      if (/^\d+$/.test(s)) return false;
      if (/^[a-f0-9]{3,8}$/i.test(s) && s.length <= 6) return true;
    }
    return false;
  }
  ...
  generateAllSelectors(element) {
    ...
    // app-select[elementid] (Angular) — стабильнее UniqueSelectorLite (.form-input app-select), иначе плеер цепляет неверный combobox
    const appSelectHost =
      element.tagName && element.tagName.toLowerCase() === 'app-select'
        ? element
        : (element.closest && element.closest('app-select'));
    if (appSelectHost) {
      const elid = appSelectHost.getAttribute('elementid') || appSelectHost.getAttribute('ng-reflect-element-id');
      const lbl = appSelectHost.getAttribute('label') || appSelectHost.getAttribute('ng-reflect-label');
      if (elid) {
        const escId = this.escapeSelector(elid);
        const selIdOnly = `app-select[elementid="${escId}"], app-select[ng-reflect-element-id="${escId}"]`;
        selectors.push({
          type: 'angular-app-select-elementid',
          value: elid,
          selector: selIdOnly,
          priority: 0.11,
          isUnique: this.checkUniqueness(selIdOnly),
          isStable: true,
          source: 'angular-app-select'
        });
        if (lbl) {
          const escLb = this.escapeSelector(lbl);
          const selIdLabel = `app-select[elementid="${escId}"][label="${escLb}"], app-select[ng-reflect-element-id="${escId}"][ng-reflect-label="${escLb}"]`;
          selectors.push({
            type: 'angular-app-select-id-label',
            value: { elementId: elid, label: lbl },
            selector: selIdLabel,
            priority: 0.07,
            isUnique: this.checkUniqueness(selIdLabel),
            isStable: true,
            source: 'angular-app-select'
          });
        }
    ...
    // Приоритет 4: id (сгенерированные id — низкий приоритет, см. isLikelyUnstableId)
    if (element.id) {
      const idSel = `#${this.escapeSelector(element.id)}`;
      const unstable = this.isLikelyUnstableId(element.id);
      selectors.push({
        type: 'id',
        value: element.id,
        selector: idSel,
        priority: unstable ? 14 : 4,
        isUnique: this.checkUniqueness(idSel),
        isStable: !unstable
      });
    }

    // Placeholder (поиск, поля фильтра) — стабильнее GWT-id
    if ((element.tagName === 'INPUT' || element.tagName === 'TEXTAREA' || element.tagName === 'SELECT') && element.getAttribute('placeholder')) {
      const ph = element.getAttribute('placeholder').trim();
      if (ph.length >= 1) {
        const tag = element.tagName.toLowerCase();
        const typePart = element.tagName === 'INPUT' && element.type
          ? `[type="${this.escapeSelector(element.type)}"]`
          : '';
        const phEsc = this.escapeSelector(ph);
        const sel = typePart
          ? `${tag}${typePart}[placeholder="${phEsc}"]`
          : `${tag}[placeholder="${phEsc}"]`;
        selectors.push({
          type: 'placeholder',
          value: ph,
          selector: sel,
    ...
    // Чекбокс + name — узкий селектор, устойчивее случайного id
    if (element.tagName === 'INPUT' && element.type === 'checkbox' && element.name) {
      const nm = this.escapeSelector(element.name);
      const sel = `input[type="checkbox"][name="${nm}"]`;
      selectors.push({
        type: 'checkbox-name',
        value: element.name,
        selector: sel,
    ...
    // Приоритет 9: тег + текст (при нестабильном id и уникальном тексте — почти как стабильный id)
    const text = this.getElementText(element);
    if (text && text.length > 0) {
      ...
      const textUnique = matchingElements.length === 1;
      let tagTextPriority = 9;
      const isFormControl = tag === 'input' || tag === 'textarea' || tag === 'select';
      if (!isFormControl && element.id && this.isLikelyUnstableId(element.id) && textUnique && text.trim().length >= 2) {
        tagTextPriority = 4.1;
      }
      selectors.push({
        type: 'tag-text',
        ...
        selector: tag,
        text: text,
        priority: tagTextPriority,
        isUnique: textUnique,
        isStable: tagTextPriority < 9
      });
    }
    ...
    if (this.detectFramework() === 'angular') {
      ...
      if (appSelect) {
        ...
        if (elementId && label) {
          const escE = this.escapeSelector(elementId);
          const escL = this.escapeSelector(label);
          const specificSelector = `app-select[elementid="${escE}"][label="${escL}"], app-select[ng-reflect-element-id="${escE}"][ng-reflect-label="${escL}"]`;
          selectors.push({
            type: 'angular-component',
            value: 'app-select',
   ...
   * При равной уникальности предпочитает стабильные типы (id, data-*, class) и короткие селекторы,
   * чтобы при записи не сохранялся длинный хрупкий CSS с :nth-child и хэшами в классах.
   */
  selectBestSelector(selectors) {
    if (!selectors || selectors.length === 0) return null;

    const isFragile = (s) => {
      if (!s || !s.selector) return false;
      const fragileTypes = ['css', 'optimal-select', 'finder-lite', 'unique-selector'];
      const typeFragile = fragileTypes.includes(s.type);
      const longOrNth = s.selector.length > 120 || ((s.selector.match(/:nth-child/g) || []).length >= 2);
      return typeFragile && longOrNth;
    };

    const sorted = selectors.slice().sort((a, b) => {
      if (a.isUnique && !b.isUnique) return -1;
      if (!a.isUnique && b.isUnique) return 1;
      // Оба уникальны: предпочитаем нехрупкий и короткий
      const aFragile = isFragile(a);
      const bFragile = isFragile(b);
      if (!aFragile && bFragile) return -1;
      if (aFragile && !bFragile) return 1;
      // Нестабильный id (GWT) — после всех стабильных селекторов с тем же уровнем уникальности
      const aBadId = a.type === 'id' && a.isStable === false;
      const bBadId = b.type === 'id' && b.isStable === false;
      if (!aBadId && bBadId) return -1;
      if (aBadId && !bBadId) return 1;
      if (a.priority !== b.priority) return a.priority - b.priority;
      return (a.selector?.length || 0) - (b.selector?.length || 0);
    });
  ...
  /**
   * Fallback: XPath вида //tag[@id='value']/path → несколько CSS-вариантов (любой тег: div, section и т.д.)
   * Поддерживает длинные пути с кастомными элементами (app-select и др.) и индексами div[2].
   * @param {string} selectorStr
   * @returns {Element|null}
   */
  _xpathIdPathToCssFallback(selectorStr) {
    if (!selectorStr || typeof selectorStr !== 'string') return null;
    // //tag[@id='id']/path (любой тег, не только div)
    const match = selectorStr.match(/^\/\/(\w+)\s*\[\s*@id\s*=\s*['"]([^'"]*)['"]\s*\]\s*\/\s*(.+)$/);
    if (!match) return null;
    const idVal = match[2];
    let rest = match[3].trim();
    if (!idVal) return null;

    const cssEsc = idVal.replace(/([ #.;,:\[\]<>+~'"\\^$|])/g, '\\$1');
    const pathToCss = (pathStr, sep) => {
      const segments = pathStr.split('/').map(s => s.trim().replace(/\[\s*(\d+)\s*\]/, ':nth-of-type($1)')).filter(Boolean);
      return segments.join(sep || ' > ');
    };
    const pathDirect = pathToCss(rest, ' > ');
    const pathDesc = pathToCss(rest, ' ');
    const trySelectors = [
      pathDirect ? `#${cssEsc} > ${pathDirect}` : `#${cssEsc}`,
      pathDesc ? `#${cssEsc} ${pathDesc}` : null,
      pathDesc ? `[id="${idVal}"] ${pathDesc}` : null,
      pathDesc ? `[id^="${idVal}"] ${pathDesc}` : null
    ].filter(Boolean);
    ...
    for (const sel of trySelectors) {
      try {
        ...
        if (el && (el instanceof Element || el instanceof HTMLElement)) return el;
      } catch (e) { /* ignore */ }
    }
    return null;
  }
  ...
  findElementSync(selectorData) {
    const selectorStr = selectorData?.selector || selectorData?.value;
    if (!selectorData || !selectorStr) return null;

    try {
      let element = null;
      const type = selectorData.type || (typeof selectorStr === 'string' && (selectorStr.startsWith('/') || selectorStr.startsWith('(')) ? 'xpath' : 'css');
      ...
      switch (type) {
        ...
        case 'aria-label':
          element = document.querySelector(selectorData.selector);
          if (!element && selectorData.value) {
            const val = String(selectorData.value);
            const stable = val.split(/\s+[−‑–—]\s+/)[0].trim() || val.slice(0, 22);
            if (stable.length >= 3) {
              const esc = stable.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
              try {
        ...
        case 'xpath': {
          ...
          if (!element && typeof selectorStr === 'string') {
            element = this._xpathIdPathToCssFallback(selectorStr);
          }
          break;
        }

        default:
          // Строка, похожая на XPath — пробуем evaluate, иначе querySelector
          if (typeof selectorStr === 'string' && (selectorStr.startsWith('/') || selectorStr.startsWith('('))) {
            try {
              const result = document.evaluate(
                selectorStr,
                document,
  ...
  findAlternativeSelectors(element, maxDepth = 3) {
    ...
    while (current && depth < maxDepth) {
      ...
      if (current.id) {
        const unstable = this.isLikelyUnstableId(current.id);
        const selector = {
          type: 'id',
          value: current.id,
