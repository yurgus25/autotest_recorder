...
(function() {
  if (window.__autotestSelectorOptimizer || window.selectorOptimizer) return;
  window.__autotestSelectorOptimizer = true;
...
class SelectorOptimizer {
  ...
  calculateSelectorScore(selector, element) {
    ...
    if (this.settings.penalizeDynamicParts) {
      ...
      // GWT / сгенерированные id (#id3c, #id12ab, #rc_select_0, #gwt-uid-1)
      if (/^#id[a-f0-9]{1,14}$/i.test(selectorStr)) score -= 42;
      if (/^#rc_select_\d+$/i.test(selectorStr) || /^#gwt-uid-\d+$/i.test(selectorStr)) score -= 40;
      if (/^#gwt-/i.test(selectorStr) || /^#ext-gen/i.test(selectorStr)) score -= 38;
    }

    if (selector.type === 'placeholder' || selector.type === 'checkbox-name' || selector.type === 'tag-text') {
      score += 12;
    }
    if (selector.isStable === false && selector.type === 'id') {
      score -= 35;
    }
    ...
    if (selectorStr.includes('[placeholder=')) {
      score += 18;
    }
