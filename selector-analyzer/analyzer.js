...
class SelectorAnalyzer {
  ...
  async init() {
    // Получаем tabId из URL (может отсутствовать при прямом открытии)
    ...
    document.getElementById('tabSelect').addEventListener('change', (e) => this.onTabSelectChange(e));
    ...
    // Вкладки (табы контента: Все / Dropdown / и т.д.)
    ...
    // Загружаем список открытых вкладок и подставляем выбранную
    await this.loadOpenTabs();
    if (!this.tabId) {
      const msg = document.documentElement.lang === 'en'
        ? '👆 Select a tab above to analyze selectors'
        : '👆 Выберите вкладку выше для анализа селекторов';
      document.getElementById('results').innerHTML = `<div class="empty-state">${msg}</div>`;
      return;
    }
    await this.analyze();
  }

  /** Заполняет выпадающий список открытых вкладок (кроме страницы анализатора). */
  async loadOpenTabs() {
    const select = document.getElementById('tabSelect');
    if (!select) return;
    const analyzerOrigin = window.location.origin;
    try {
      const tabs = await chrome.tabs.query({});
      const injectable = tabs.filter(tab => {
        if (!tab.id || !tab.url) return false;
        if (tab.url.startsWith('chrome://') || tab.url.startsWith('edge://')) return false;
        if (tab.url.startsWith(analyzerOrigin) && tab.url.includes('selector-analyzer/')) return false;
        return true;
      });
      select.innerHTML = '';
      const isRu = document.documentElement.lang !== 'en';
      if (injectable.length === 0) {
        select.innerHTML = isRu ? '<option value="">— Нет доступных вкладок —</option>' : '<option value="">— No tabs available —</option>';
        return;
      }
      const selectedId = this.tabId ? String(this.tabId) : null;
      let hasSelection = false;
      injectable.forEach(tab => {
        ...
        if (selectedId && String(tab.id) === selectedId) {
          opt.selected = true;
          hasSelection = true;
        }
      });
      if (!hasSelection && injectable.length > 0) {
        select.selectedIndex = 0;
        this.tabId = String(select.value);
        this.updateUrlTabId(this.tabId);
      } else if (selectedId) {
        this.tabId = selectedId;
      }
    } catch (e) {
      console.error('Ошибка загрузки вкладок:', e);
      const isRu = document.documentElement.lang !== 'en';
      select.innerHTML = isRu ? '<option value="">— Ошибка загрузки —</option>' : '<option value="">— Load error —</option>';
    }
  }
  ...
  onTabSelectChange(e) {
    ...
    if (!value) return;
    this.tabId = value;
    this.updateUrlTabId(value);
    this.analyze();
  }

  updateUrlTabId(tabId) {
